<template>
  <div class="simpson-page">
    <h1 class="page-title">Simpson's Rule Method</h1>
    ...
    <div class="content-container">
      ...
      <div class="input-section">
        ...
        <select v-model="method">
          <option value="single">Single Simpson's 1/3</option>
          <option value="composite">Composite Simpson's 1/3</option>
          <option value="threeEight">Simpson's 3/8 Rule</option>
        </select>

        <div v-if="method === 'composite' || method === 'threeEight'">
...
export default {
  ...
  methods: {
    ...
    getExact() {
      ...
      for (let i = 1; i < n; i++) sum += 2 * this.f(this.a + i * h);
      return (h / 2) * sum;
    },
    calculate() {
      this.steps = [];
      ...
      if (this.method === "single") {
        const h = (this.b - this.a) / 2;
        const fa = this.f(this.a);
        const fm = this.f((this.a + this.b) / 2);
        const fb = this.f(this.b);
        this.steps.push(`h = (b - a)/2 = ${h}`);
        this.steps.push(`f(${this.a}) = ${fa}`);
        this.steps.push(`f(mid) = f(${(this.a + this.b) / 2}) = ${fm}`);
        this.steps.push(`f(${this.b}) = ${fb}`);
        approx = (h / 3) * (fa + 4 * fm + fb);
        this.displayEquation = `(h/3) * [ f(a) + 4f(mid) + f(b) ]`;
      } else if (this.method === "composite") {
        if (this.n % 2 !== 0) {
          alert("n must be even number for Simpson's 1/3");
          return;
        }
        const h = (this.b - this.a) / this.n;
        let sum = this.f(this.a) + this.f(this.b);
        this.steps.push(`h = (b - a)/n = (${this.b} - ${this.a})/${this.n} = ${h}`);
        ...
        this.steps.push(`x${this.n} = ${this.b}, f(x${this.n}) = ${this.f(this.b)}`);
        approx = (h / 3) * sum;
        this.displayEquation = `(h/3) * [ f(x0) + 4Σf(odd) + 2Σf(even) + f(xn) ]`;
      } else if (this.method === "threeEight") {
        if (this.n % 3 !== 0) {
          alert("n must be divisible by 3 for Simpson's 3/8");
          return;
        }
        const h = (this.b - this.a) / this.n;
        let sum = this.f(this.a) + this.f(this.b);
        ...
        this.steps.push(`x${this.n} = ${this.b}, f(x${this.n}) = ${this.f(this.b)}`);
        approx = (3 * h / 8) * sum;
        this.displayEquation = `(3h/8) * [ f(x0) + 3Σf(not multiple of 3) + 2Σf(multiples of 3) + f(xn) ]`;
      }
