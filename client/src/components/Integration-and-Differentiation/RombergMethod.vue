<template>
  <div class="romberg-page">
    <h1 class="page-title">Romberg Integration Method</h1>
    ...
    <div class="content-container">
      ...
      <div class="input-section">
        <label>Level of Romberg (k):</label>
        <input type="number" v-model.number="maxK" min="1" />
      </div>
      ...
      <div v-if="rombergTable.length" class="result-container">
        <h3>Result:</h3>
        <p>
          I = ∫<sub>{{ a }}</sub><sup>{{ b }}</sup> f(x) dx
        </p>
        <p><b>Exact:</b> {{ exact }}</p>
        <p><b>Romberg Approximation:</b> {{ finalResult }}</p>
        <p><b>Error (%):</b> {{ relError }}</p>
        <p><b>Equation:</b> Romberg Extrapolation</p>

        <div class="steps">
          <h4>Romberg Table:</h4>
          <table class="romberg-table">
            <tr v-for="(row, i) in rombergTable" :key="i">
...
export default {
  ...
  computed: {
    latexExpr() {
      ...
      return `I = \\int_{${this.a}}^{${this.b}} ${expr}\\, dx`;
    },
  },
  watch: {
    a() {
  ...
  methods: {
    ...
    calculate() {
      this.rombergTable = [];
      let exactVal = this.getExact();

      for (let k = 0; k <= this.maxK; k++) {
        let row = [];
        const n = 2 ** k;
        const h = (this.b - this.a) / n;
        const Tk = this.trapezoidal(h, n);

        row.push(Tk.toFixed(6));

        for (let m = 1; m <= k; m++) {
          const prev = parseFloat(this.rombergTable[k - 1][m - 1]);
          const Rkm1 = parseFloat(row[m - 1]);
          const Rkm = Rkm1 + (Rkm1 - prev) / (Math.pow(4, m) - 1);
          row.push(Rkm.toFixed(6));
        }
        this.rombergTable.push(row);
      }

      this.finalResult =
        this.rombergTable[this.rombergTable.length - 1].slice(-1)[0];

      this.exact = exactVal.toFixed(6);
      const relErr =
        (Math.abs(exactVal - this.finalResult) / Math.abs(exactVal)) * 100;
