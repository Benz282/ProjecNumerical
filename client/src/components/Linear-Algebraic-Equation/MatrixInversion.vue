<template>
  <div class="matrix-inversion-page">
    <h1 class="page-title">Matrix Inversion Method</h1>
    ...
    <div class="content-container">
      ...
      <div v-if="A.length > 0" class="matrix-section">
        ...
        <button class="calculate-btn" @click="matrixInversion">Calculate</button>
      </div>
      ...
      <div v-if="inverse.length > 0" class="result-container">
        <h3>Inverse Matrix (A⁻¹):</h3>
        <table>
          <tr v-for="(row, i) in inverse" :key="i">
            <td v-for="(val, j) in row" :key="j">
              {{ val.toFixed(6) }}
            </td>
          </tr>
...
export default {
  ...
  methods: {
    ...
    matrixInversion() {
      ...
      this.steps = [];
      this.steps.push({
        matrix: this.snapshotMatrix(aug),
      ...
      this.inverse = aug.map((row) => row.slice(n));
      this.X = this.inverse.map((row) =>
