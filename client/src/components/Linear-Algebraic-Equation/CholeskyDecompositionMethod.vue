<template>
  <div class="cholesky-page">
    <h1 class="page-title">Cholesky Decomposition Method</h1>
    ...
    <div class="content-container">
      ...
      <div v-if="A.length > 0" class="matrix-section">
        ...
        <button class="calculate-btn" @click="choleskyDecomposition">
          Calculate
        </button>
      </div>

      <div v-if="L.length > 0" class="result-container">
        ...
        <br />
        <h3>Upper Triangular Matrix (Lᵀ):</h3>
        <table>
          <tr v-for="(row, i) in LT" :key="i">
            <td v-for="(val, j) in row" :key="j">
              {{ val.toFixed(6) }}
            </td>
          </tr>
      ...
      <div v-if="X.length > 0" class="result-container">
        <h3>Backward Substitution (solve Lᵀ·X = Y):</h3>
        <table>
          <thead>
            <tr>
              <th>xi</th>
...
<script>
// The code below is unchanged from your original provided code.
export default {
  ...
  methods: {
    ...
    choleskyDecomposition() {
      ...
      if (allZeroA) {
        alert("Please Input all values in Matrix A and Matrix B ");
        return;
      }
      const n = this.n;
      ...
      for (let i = 0; i < n; i++) {
        for (let j = 0; j <= i; j++) {
          ...
          if (i === j) {
            ...
            if (val <= 0) {
              alert(
                "Matrix must be Symmetric and Positive Definite for Cholesky Decomposition."
              );
              return;
            }
            L[i][j] = Math.sqrt(val);
      ...
      let LT = Array.from({ length: n }, (_, i) =>
