<template>
  <div class="jacobi-page">
    <h1 class="page-title">Jacobi Iteration Method</h1>
    ...
    <div class="content-container">
      ...
      <div v-if="A.length > 0" class="matrix-section">
        ...
        <button class="calculate-btn" @click="jacobiIteration">
          Calculate
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    jacobiIteration() {
      ...
      if (allZeroA ) {
        alert(
          "Please Input all values in Matrix A, Matrix B and Initail X (X₀)."
        );
        return;
      }
      const n = this.n;
      ...
      for (let iter = 0; iter < this.maxIter; iter++) {
        ...
        error = Math.max(
          ...X_new.map((xi, i) => Math.abs((xi - X[i]) / (xi || 1)) * 100)
        );
        iterations.push({
          x: [...X_new],
